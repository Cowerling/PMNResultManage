(function ($) {
    ...
    $.fn.createDataListTable = function (url, admin, entire) {
        ...
        data_table.on("draw.dt", function () {
            if (admin) {
                $(this).find(".table-tools").append("<a class='data-record-delete' href='#'><i class='fa fa-trash-o table-delete-tool'></i></a>");
            }

            $(this).find(".table-tools").hide();

            $(this).find(".data-record-delete").click(function (event) {
                event.preventDefault();
                var data_record_tag = $(this).prev().attr("href");
            ...
            ajax: {
                ...
                dataSrc: function (result) {
                    ...
                    for (var i =0, length = result.dataRecords.length; i < length; i++) {
                        result.dataRecords[i].name = result.dataRecords[i].name + "<span class='table-tools'><a href='" + result.dataRecords[i].tag + "'>\u8be6\u60c5</a></span>"
