(function ($) {
    ...
    $.fn.createProjectListTable = function (url, admin) {
        var data_table = $(this);
    ...
    $.fn.createDataListTable = function (url, admin, entire) {
        var data_table = $(this);

        data_table.on("draw.dt", function () {
            if (admin) {
                $(this).find(".table-tools").append("<a class='data-record-authority' href='#'>\u6743\u9650</a>");
            }

            $(this).find(".data-record-authority").click(function (event) {
                event.preventDefault();
                var data_record_tag = $(this).prev().val("href");
                alert(data_record_tag);
            ...
            ajax: {
                ...
                dataSrc: function (result) {
                    ...
                    for (var i =0, length = result.dataRecords.length; i < length; i++) {
                        result.dataRecords[i].name = result.dataRecords[i].name + "<input type='hidden' value='" + result.dataRecords[i].tag + "'></input>"
                        ...
                        var operation = "";
                        for (j = 0, operation_length = result.dataRecords[i].authorities.length; j < operation_length; j++) {
                            var operation_name;
                            var operation_color_style;
                            switch (result.dataRecords[i].authorities[j]) {
                                case "VIEW":
                                    operation_name = "\u67e5\u9605";
                                    operation_color_style = "label-success";
                                    break;
                                case "DOWNLOAD":
                                    operation_name = "\u4e0b\u8f7d";
                                    operation_color_style = "label-success";
                                ...
                                case "DELETE":
                                    operation_name = "\u5220\u9664";
                                    operation_color_style = "label-danger";
                                    break;
                                default:
                                    break;
                            }
                            result.dataRecords[i].operation +=
                                "<span class='label table-status " + operation_color_style + "'>" +
                                "<a class='text-white' href='#'>" + operation_name + "</a>" +
                                "</span>"
                        }
                        result.dataRecords[i].operation = operation
                    }
