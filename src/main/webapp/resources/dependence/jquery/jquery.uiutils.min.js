(function ($) {
    ...
    $.fn.createUserSelectModal = function (title, url, input_user_name) {
        var modal = $(this);
        ...
        user_list_tree.initUserListTree(false, user_search, search_content);
    ...
    $.fn.createMultiUsersSelectModal = function () {
        ...
        $(this).on("show.bs.modal", function (event) {
            ...
            user_list_tree.on("nodeSelected", function(event, node) {
                var input_user_content = input_user.val() != "" ? input_user.val().split(";") : [];
                input_user_content.push(node.alias);
                input_user.val(input_user_content.join(";"));

                user_message.text("\u9009\u4e2d: " + input_user_content.length + "\u4eba");

                user_unselected.hide();
            });
            user_list_tree.on("nodeUnselected", function(event, node) {
                var input_user_content = input_user.val().split(";");
                input_user_content.splice(input_user_content.indexOf(node.alias), 1);
                input_user.val(input_user_content.join(";"));

                user_message.text(input_user_content.length != 0 ? "\u9009\u4e2d: " + input_user_content.length + "\u4eba": "");
            });
            user_list_tree.on("nodeExpanded", function(event, node) {
                if (user_list_tree.hasClass("search")) {
                    user_list_tree.treeview("search", [search_content.val(), {
